...
// Get users from GitHub repo
export async function getUsers(token, owner, repo, path = 'users.json') {
  try {
    const response = await fetch(
      `https://api.github.com/repos/${owner}/${repo}/contents/${path}`,
      {
...
export async function saveUsers(token, owner, repo, users, path = 'users.json', message = 'Update users') {
  try {
    // First, get the current file to get its SHA
    const getFileResponse = await fetch(
      `https://api.github.com/repos/${owner}/${repo}/contents/${path}`,
